    ...
    <table class="table" id="applyApiTable">
      ...
      <template v-if="list.length > 0">
        <tr v-for="(item, index) in list">
          ...
          <td>
            <a href="javascript:;" @click="showView(item.id, item.dataId, item.serviceName)">查看</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      apply(item) {
        this.$router.push({name: 'resCatalogDetail',params:{id:item.dataId}})
        ...
        //       id: item.dataId, type: 'api', name: item.serviceName,
      ...
      update(item) {
        this.$router.push({name:'resApplyUpdate', params: {id: item.id,type:'cata',name:item.serviceName,
          depName:item.dep_name,description:item.serviceName}})
      },
      showView(id, dataId,name) {
...
//        this.getDetail();
      ...
      apply1() {
        ...
        if (this.detail.shareType === '01') {
          ...
          this.$router.push({
            name: 'resApply', params: {
              id: this.dataId, type: 'data', name: this.detail.serviceName,
