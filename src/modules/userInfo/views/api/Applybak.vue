    ...
    <table class="table" id="applyApiTable">
      ...
      <template v-if="list.length > 0">
        <tr v-for="(item, index) in list">
          ...
          <td>{{ cutString(item.description,18) }}</td>
          ...
          <td>
            <a href="javascript:;" @click="showView(item.id, item.serviceId)">查看</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      apply(item) {
        this.$router.push({name: 'apiDetail',params:{id:item.serviceId}})
