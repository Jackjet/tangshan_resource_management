    ...
    <table class="table" id="applyApiTable">
      ...
      <template v-if="list.length > 0">
        <tr v-for="(item, index) in list">
          ...
          <td>
            <a href="javascript:;" @click="showView(item.id, item.serviceId, item.name)">查看</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      queryList() {
        API.Data.getCheckMyUseDataList(this.params).then((res) => {
      ...
      apply(item) {
        ...
        this.layerIndex = layer.open({
          ...
          btn1: (index) => {
            ...
            let data={comments:temp,checkStatus:"true",id:this.commmonParam.id,pageType:pageType}
            API.Data.doApplyCheck(data).then((res) => {
              if (res.status === 'OK') {
                layer.msg('通过成功！')
                this.queryList()
              }
          ...
          btn2: (index) => {
            if(this.content){
              ...
              let data={comments:this.content,checkStatus:"false",id:this.commmonParam.id,pageType:pageType}
              API.Data.doApplyCheck(data).then((res) => {
                if (res.status === 'OK') {
                  layer.msg('驳回成功！')
                  this.queryList()
      ...
      downloadAgreement(id) {
        API.Data.downloadAgreement({applyId:id}).then(res => {
          if(res.code=="000000"){
            if(res.result){
