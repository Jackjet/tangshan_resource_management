...
//      })
      API.Common.querydetail({id:this.$route.params.id}).then(res => {
//        this.param = res.result
        this.param.busType=res.result.bus_type
        if(res.result.purpose){
          let re =new RegExp(",","g");
          this.param.purpose=res.result.purpose.replace(re,"");
        }
        this.times=[this.formatDateTime(res.result.startTime),this.formatDateTime(res.result.endTime)]
        this.param.applyUserName=res.result.apply_user_name
        this.param.applyUserPhone=res.result.apply_user_phone
        this.param.applyUserTel=res.result.apply_user_tel
        this.param.applyUserEmail=res.result.apply_user_email
        this.param.busId=res.result.busId
        this.param.itemreq=res.result.itemreq
        this.param.busreq=res.result.busreq
        this.param.explains=res.result.explains
        this.param.id=res.result.id
        var that=this;
        if(res.result.files){
          res.result.files.forEach(function (item, index) {
            item.name=item.fileName+"."+item.fileType
            that.fileList.push(item)
            that.param.fileList.push(item)
          });
        }
    })
...
//        var isHasActiveTag = $(".apply_declare_tag_group .el-tag").hasClass('active')
        ...
        var res=await axios.post('/ro/share-common-api/userApply/updateApply.do',this.param);
        if(res.code==='000000'){
          layer.msg(res.result);
          if(res.status==='OK') {
            if (this.param.busType === '01') {
              this.$router.push({name: 'userApiApply', params: {id: this.param.busId}});
            } else if (this.param.busType === '02') {
              this.$router.push({name: 'userDataApply', params: {id: this.param.busId}});
            }
          }
        }
      },
