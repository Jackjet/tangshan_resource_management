<template>
  <div class="result-item">
    <div v-if="apiItem.status === '0'" class="item-icon">
      <router-link :to="{ name: 'dataDetail', params: {id: apiItem.id}}">
        <img class="apiItemImg" :src="apiItem.dataPic"/>
      </router-link>
      <div class="result-item_detail">
        <div class="title_info">
          ...
          <div class="detail_attention">
            ...
            <span>{{apiItem.viewCount||0}}</span>
            <img src="../../assets/images/icons/ico_02.svg" title="关注量">
            <span>{{apiItem.attentionCount||0}}</span>
            <img src="../../assets/images/icons/ico_03.svg" title="下载量" v-if="type===0">
            <img src="../../assets/images/icons/ico_04.svg" title="接入量" v-else>
            <span v-if="type===0">{{apiItem.downloadCount||0}}</span>
            <span v-else>{{apiItem.applyCount||0}}</span>
          </div>
        </div>
        <div class="condition_label">
          <div v-for="item in apiItem.tagList">
        ...
        <div class="detail_info" v-if="type===0">
          <el-row>
            <div class="lineHeight">
              ...
              <el-col :span="12">发布日期：{{apiItem.createdTime|dateformat('YYYY年MM月DD日')}}</el-col>
            </div >
            <div class="lineHeight">
              <el-col :span="24" v-if="apiItem.strRes===''">关联目录：无</el-col>
              <el-col :span="24" v-else>关联目录：{{apiItem.cataName}}</el-col>
            </div>
            <div class="lineHeight">
              <el-col :span="24" >文件描述：<span class="fileDes">{{apiItem.description}}</span>
              </el-col>
            </div>
          </el-row>
      </div>
...
<script>
  export default {
    props: ['item', 'type'],// type 云数据0 云接口1 云服务2
    data() {
      return {
        apiItem: this.item,
        //云数据0 云接口1 云服务2
